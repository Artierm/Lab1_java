...
public class Formula extends JFrame {
    ...
    public Double calculate1(Double x, Double y, Double z) {
        ...
        return Math.pow(Math.sin(y),(Math.log(Math.pow(2,1+x)))+Math.cos(3.14*z*z*z)+Math.pow(1/x,((Math.pow(Math.pow(x,2) ,Math.exp(1))))+Math.cos(Math.pow(Math.exp(1),z))+Math.sqrt(1/y)));
    ...
    public Double calculate2(Double x, Double y, Double z) {
        ...
        if (z <=0 || x < 0){
            JOptionPane.showMessageDialog(Formula.this,
                    "Отрицательное число под корнем чётной степени", "Ошибочный формат числа", JOptionPane.WARNING_MESSAGE);
            return 0.0;
        }
        if (y == 0 || x == -1){
            JOptionPane.showMessageDialog(Formula.this,
        ...
        return (x*(Math.pow(3,Math.cos(x))*y*y))/Math.pow(1/x,z);
    }
